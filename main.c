...
#include <time.h>
#include <math.h>
#include "lista.h" // Assumindo que este é o header do outro programa
...
void novoMenu(Corredor corredores[], LISTA_TXT* listaResultados, int* qtdCorredores) {
    ...
    switch (opcao) {                                                                              
        case 1:
            exibirResultados(corredores, *qtdCorredores); // Assuma que esta função já está implementada
            break;
        case 2:
            salvarResultados(corredores, *qtdCorredores, listaResultados); // Implemente esta função para salvar os resultados
            break;
        case 3:
            if (refazerCorrida()) {
                ...
                return; // Importante para sair da função atual e evitar loop infinito
            }
            break;
        case 4:
            printf("Saindo...\n");
...
void simularCorrida(Corredor corredores[], LISTA_TXT* listaResultados, int* qtdCorredores) {
    ...
    ordenarCorredores(corredores, *qtdCorredores); // Assuma que esta função já está implementada
...
void salvarResultados(Corredor corredores[], int qtdCorredores, LISTA_TXT* listaResultados) {
    ...
    inserirElementoLista(listaResultados, nomeArquivo); // Esta função deve ser implementada no outro programa
}
